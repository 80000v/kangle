...
#ifndef NDEBUG
	rq->ctx->upstream_socket = client->socket->get_socket();
#endif
	if (!TEST(rq->flags,RQ_NET_BIG_OBJECT)) {
		rq->c->removeSocket();
	}
...
void KAsyncFetchObject::handleUpstreamError(KHttpRequest *rq,int error,const char *msg,int last_got)
{
	...
	klog(KLOG_INFO,"rq = %p connect to %s:%d error code=%d,msg=[%s],try count=%d,last errno=[%d %s],socket=%d(%s) last_got=%d (%d %d)\n",
		(KSelectable *)rq,
		ips,
