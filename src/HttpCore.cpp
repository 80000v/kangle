...
KWStream *makeWriteStream(KHttpRequest *rq, KHttpObject *obj, KWStream *st,
	...
	if (TEST(obj->index.flags,OBJ_DEFLATED)) {
		//上游是deflate压缩，则解压
		CLR(obj->index.flags,OBJ_DEFLATED);
		KHttpStream *st2 = new KGzipDecompress(true,st, autoDelete);
		st = st2;
		autoDelete = true;
	}
	if (TEST(obj->index.flags,OBJ_GZIPED)
		&& (rq->needFilter() || !TEST(rq->flags,RQ_HAS_GZIP))) {
		//两种情况需要解压内容
		//一种是需要内容过滤。
		//另一种是即使没有发送gzip(下游并不支持gzip)到upstream中,upstream也会假定我们支持gzip，比如spdy协议。
		//解压了就把压缩志清掉
		//printf("加载dgzip层\n");
...
bool build_obj_header(KHttpRequest *rq, KHttpObject *obj,
	...
	//gzip压缩
	if (TEST(rq->flags,RQ_HAS_GZIP) && TEST(obj->index.flags,OBJ_GZIPED)) {
...
bool processCacheRequest(KHttpRequest *rq) {
	KHttpObject *obj = rq->ctx->obj;
...
void asyncLoadHttpObject(KHttpRequest *rq) {
	...
	} else if (context->old_obj && !TEST(context->old_obj->index.flags,OBJ_NOT_OK|OBJ_IS_DELTA)) {
